...
@implementation NetworkTaskEx

+ (NetworkTaskEx *)sharedNetworkTask {
    
    static NetworkTaskEx *netTask = nil;
...
- (void)requestWithMethod:(NSString *)method
    ...
    __weak NetworkTaskEx *weakSelf = self;
    NSString * urlString = [NSString stringWithFormat:@"%@/%@",kNetworkAPIServer,api];
    if ([method isEqualToString:@"GET"]) {
        
        [_afManager GET:urlString parameters:param success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
            //
        ...
        [_afManager POST:urlString parameters:param success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
            //
...
- (void)startGETTaskURL:(NSString*)urlString
    ...
    [_afManager GET:urlString parameters:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
        //
        [weakSelf analyzeData:responseObject delegate:delegate resultObj:resultObj customInfo:customInfo];
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        //
