...
+ (BOOL)createZipFileAtPath:(NSString *)path withDirectory:(NSArray *)directoryPath
{
    BOOL success = NO;
    ...
    if ([zipArchive open]) {
        ...
        for (NSString * curDirPath in directoryPath) {
            NSDirectoryEnumerator *dirEnumerator = [fileManager enumeratorAtPath:curDirPath];
            NSString *fileName;
            while ((fileName = [dirEnumerator nextObject])) {
                BOOL isDir;
                NSString *fullFilePath = [curDirPath stringByAppendingPathComponent:fileName];
                [fileManager fileExistsAtPath:fullFilePath isDirectory:&isDir];
                if (!isDir) {
                    fileName = [[curDirPath lastPathComponent] stringByAppendingPathComponent:fileName];
    ...
    if ([zipArchive open]) {
        ...
        while ((fileName = [dirEnumerator nextObject])) {
            ...
            else
            {
                if([[NSFileManager defaultManager] subpathsOfDirectoryAtPath:fullFilePath error:nil].count == 0)
                {
                    NSLog(@"sub paths of %@ is empty", fullFilePath);
                    NSString *tempName = [fullFilePath stringByAppendingPathComponent:@".DS_Store"];
                    [@"" writeToFile:tempName atomically:YES encoding:NSUTF8StringEncoding error:nil];
                    if (keepParentDirectory)
                    {
                    ...
                    NSLog(@"sub paths of %@ is not empty", fullFilePath);
                }
            }
        }
        success = [zipArchive close];
...
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(NSString *)fileName
{
    ...
    NSLog(@"write file:%@ to path:%@", fileName, path);
...
//    FILE *input = fopen([path UTF8String], "r");
    ...
    NSUInteger len = 0;
    NSUInteger totalLen = fileContent.length;
    for (NSUInteger i = 0; i < totalLen; i++) {
        ...
        [fileContent getBytes:buffer range:NSMakeRange(i, len)];
...
+ (NSDate *)__dateWithMSDOSFormat:(UInt32)msdosDateTime {
	static const UInt32 kYearMask = 0xFE000000;
	static const UInt32 kMonthMask = 0x1E00000;
	static const UInt32 kDayMask = 0x1F0000;
	static const UInt32 kHourMask = 0xF800;
...
- (BOOL)_writeData:(NSData *)data filename:(NSString *)filename {
    if (!_zip) {
		return NO;
    }
    if (!data) {
